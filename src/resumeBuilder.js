...
function downloadPDF(atsMode) {
  // 1. Setup - get references and template class
  const preview = document.getElementById('resumePreview');
  const templateClass = atsMode ? 'ats' : (data.template || 'midnight');
  
  // 2. Fetch all valid pages generated by your paginateResume function
  ...
  if (pages.length === 0) {
    if(typeof showToast === 'function') showToast('No content to export.', 'error');
    return;
  }

  // 3. Hide the entire application UI temporarily
  const originalBodyVisibility = [];
  Array.from(document.body.children).forEach(child => {
    if (child.id !== 'real-pdf-print-wrapper' && child.tagName !== 'SCRIPT' && child.tagName !== 'STYLE') {
      originalBodyVisibility.push({ el: child, display: child.style.display });
  ...
  `;
  printWrapper.appendChild(printStyles);

  // 7. Clone pages into the mock wrapper
  pages.forEach(page => {
  ...
  printWrapper.appendChild(mockPreview);
  ...
  setTimeout(() => {
    ...
    // 10. Cleanup after the native print dialog closes
    setTimeout(() => {
