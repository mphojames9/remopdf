...
export default function MonolithBrutalist({
  ...
  // Separated Web links to ensure all are rendered if present
  if (info.github) {
    contactItems.push(
  ...
  // Section Heading Builder (Updated with Premium Color)
  const renderSectionHeader = (title) => (
    <div className="w-full mt-4 mb-2.5 border-b-[1.5px] pb-0.5 shrink-0" style={{ borderColor: accentColor }}>
        ...
        {isFlatStringList ? (
          ...
          <div className="space-y-1">
            {validSkills.map((skill, idx) => {
              ...
              return (
                <div key={`skill-${idx}`} className="text-[12px] text-gray-900 leading-[1.5]">
        ...
        {validExperience.map((exp, idx) => (
          <div key={`exp-${idx}`} className="w-full mb-3 break-inside-avoid">
            ...
            {exp.achievements && exp.achievements.length > 0 ? (
              <ul className="list-disc list-outside ml-5 text-[11.5px] text-gray-800 space-y-1 leading-[1.55]">
                {exp.achievements.map((ach, i) => (
                  <li key={i} className="pl-0.5">{ach}</li>
                ))}
              </ul>
            ) : exp.description ? (
              <div className="text-[11.5px] text-gray-800 leading-[1.6] whitespace-pre-wrap ml-1">
                {exp.description}
              </div>
            ) : null}
          </div>
      ...
      <div key="section-education" className="w-full mb-3 shrink-0">
        ...
        {validEducation.map((edu, idx) => (
          <div key={`edu-${idx}`} className="w-full mb-3 break-inside-avoid">
            ...
            {edu.description && (
              <div className="text-[11.5px] text-gray-800 mt-1 leading-[1.5] whitespace-pre-wrap">
                {edu.description}
              </div>
            )}
          </div>
  ...
  if (validCertificates && validCertificates.length > 0) {
    mainElements.push(
      <div key="section-certifications" className="w-full mb-3 shrink-0">
        ...
        <ul className="list-disc list-outside ml-5 text-[11.5px] text-gray-800 space-y-2 leading-[1.55]">
          {validCertificates.map((cert, idx) => (
            <li key={`cert-${idx}`} className="pl-0.5">
              ...
              {cert.description && (
                <div className="mt-0.5 whitespace-pre-wrap text-gray-800 leading-[1.6]">
                  {cert.description}
                </div>
              )}
            </li>
  ...
  if (validLanguages && validLanguages.length > 0) {
    ...
    mainElements.push(
      <div key="section-languages" className="w-full mb-3 shrink-0">
        ...
        <div className="text-[11.5px] text-gray-900 leading-[1.5]">
          {langStr}
        </div>
      </div>
    );
      ...
      <div key="section-references" className="w-full mb-3 shrink-0">
        ...
        <div className="grid grid-cols-2 gap-4">
          {validReferences.map((ref, idx) => (
            <div key={`ref-${idx}`} className="text-[11.5px] text-gray-900 leading-[1.5] break-inside-avoid">
