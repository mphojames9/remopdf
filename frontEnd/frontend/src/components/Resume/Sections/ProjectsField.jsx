...
// FIX: Added `onNext` to the props here!
  ...
  return (
    <div className="w-full max-w-3xl mx-auto font-sans animate-fade-in pb-6 sm:pb-12">
      <div className="mb-6 sm:mb-10">
        <span className="text-orange-500 text-[10px] sm:text-xs font-bold uppercase tracking-widest bg-orange-50 px-3 py-1.5 rounded-full border border-orange-100">Step 6 of 7</span>
        <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight mt-4">Projects</h2>
        <p className="text-slate-500 text-sm mt-1">Highlight key personal or professional projects.</p>
      </div>

      <div className="space-y-6">
        {projects.map((proj, index) => (
          <div key={proj.id || index} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-6 md:p-8 relative group transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mt-2">
              <InputField label="Project Name" placeholder="e.g. E-Commerce Dashboard" value={proj.title} onChange={(e) => handleUpdate(index, 'title', e.target.value)} />
              <InputField label="Project Link / GitHub" placeholder="e.g. github.com/username/project" value={proj.link} onChange={(e) => handleUpdate(index, 'link', e.target.value)} />
      ...
      <div className="mt-12 border-t border-slate-100 pt-8 flex justify-between items-center">
        <button onClick={onPrev} className="px-6 py-3 border-2 border-slate-200 text-slate-600 font-semibold rounded-xl hover:bg-slate-50 transition-all flex items-center gap-2 text-sm"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg> Back</button>
